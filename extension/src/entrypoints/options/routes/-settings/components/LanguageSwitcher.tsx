...
export function LanguageSwitcher({ language, onChange }: LanguageSwitcherProps) {
  ...
  return (
    <Listbox value={language} onChange={onChange}>
      <ListboxButton className="relative block h-12 min-w-40 cursor-pointer text-nowrap rounded-lg bg-slate-950/5 py-1.5 pr-8 pl-3 text-left text-slate-900 text-sm/6 transition hover:text-sky-500 focus:outline-none focus-visible:outline-current data-focus:outline-2 data-focus:outline-black/25 data-focus:-outline-offset-2 dark:bg-white/5 dark:text-white dark:data-focus:outline-white/25">
      ...
      >
        <div className="w-(--button-width) rounded-xl border border-black/5 bg-black/5 p-1 dark:border-white/5 dark:bg-white/5">
          {Object.keys(i18n.options.resources!).map((language) => (
            ...
            >
              ...
              <div className="overflow-hidden text-ellipsis text-nowrap text-slate-900 text-sm/6 dark:text-white">
                {getDisplayName(language)}
              </div>
            </ListboxOption>
          ))}
